"""
Evaluate per-task GrowNet ADMET classifiers on the SELFIES latent test set.

Loads the test latent pack produced by create_multitask_dataset_selfies.py
and each task's GrowNetOracle checkpoint from ckpts/grownet_admet/{variant}/,
then reports per-task and macro-averaged metrics (accuracy, F1, ROC-AUC,
PR-AUC) plus confusion matrices — same format as evaluate_multitask_selfies.py.

    python Models/AdmetClassifier/evaluate_grownet_selfies.py --variant pretrain
    python Models/AdmetClassifier/evaluate_grownet_selfies.py --variant akt1 --threshold 0.4
"""
...
# ---------------------------------------------------------------------------
# Metrics helper (same as evaluate_multitask_selfies.py)
# ---------------------------------------------------------------------------
...
def evaluate(
    ...
    if not gn_dir.exists():
        raise FileNotFoundError(
            f"GrowNet checkpoint dir not found: {gn_dir}\n"
    ...
    # Load GrowNet checkpoints
    gn_models: Dict[str, GrowNetOracle] = {}
    for pt_path in sorted(gn_dir.glob("*.pt")):
        task_name = pt_path.stem
        if task_name in tasks:
            gn_models[task_name] = GrowNetOracle.load(str(pt_path), device=device)
            gn_models[task_name].eval()

    print(
        f"\n=== GrowNet ADMET evaluation ({variant}) "
    ...
    for task_idx, task_name in enumerate(tasks):
        if task_name not in gn_models:
            print(f"  [skip] {task_name}: no checkpoint found in {gn_dir}")
            continue
        if not zs_by_task[task_idx]:
            print(f"  [skip] {task_name}: no test samples")
            continue

        model = gn_models[task_name]
        zs_np = np.stack(zs_by_task[task_idx]).astype(np.float32)   # (N, latent_dim)
        ys_np = np.array(ys_by_task[task_idx])                       # (N,)
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate per-task GrowNet ADMET classifiers.")
